...
const SCENES = {
  ...
  site: {
    ...
    objects: [
      { id: "material_pile", name: "管材下腳料", icon: "🚰", x: 330, y: 320 },
    ],
  },
  exhibition: {
    label: "特展布展現場",
...
const ITEM_NAMES = {
  gift_box: "廠商禮盒",
  material_photo: "疑似管材規格差異照片",
  spec_compare_doc: "規格比對報告",
};

// ---- 任務流程 ----
const QUEST_STEPS_CASE1 = [
  { id: "start", title: "向祥哥報到", desc: "去找祥哥，了解這次古蹟修復工程初驗作業的狀況。" },
  { id: "go_site", title: "前往工地", desc: "前往古蹟修復工地，了解目前初驗與施工狀況。" },
...
const DIALOGUES = {
  ...
  xiaofeng: {
    ...
    nodes: {
      ...
      intro2: {
        speaker: "小風",
        text: "政風不是專門在抓人的啦～我們比較像「事前的安全帶」。\n工作上遇到不確定的狀況，像是廠商送禮、採購程序等等，都歡迎隨時來找我聊聊！",
        choices: [
      ...
      c2_register_declined: {
        speaker: "小風",
        text: "聽說廠商約你吃飯，後來你婉拒了對吧？很好！\n跟職務有利害關係廠商的私人飲宴，原則上就不應參加。另外，像這種可能引發廉政疑慮的邀約，本機關政風室也建議主動來說明、留下紀錄；即使你沒有赴約，事先留下紀錄，日後若有人對雙方往來提出質疑，也比較容易把事情說清楚。",
        choices: [
          {
            label: "了解，我現在登錄",
            next: null,
  ...
  chief: {
    ...
    nodes: {
      briefing: {
        speaker: "祥哥",
        text: "喔，你來啦。OO街屋古蹟修復工程已經竣工了，監造那邊說目前看起來都沒問題。",
        next: "briefing2",
      },
      briefing2: {
        speaker: "祥哥",
        text: "這案子拖超過一年，接下來要先辦初驗。這次初驗你協助辦理，程序能跑就盡快跑一跑，後面的驗收也比較好接著辦，大家都輕鬆～",
        choices: [
          {
            label: "我了解，我先去工地看看狀況",
            next: null,
      ...
      final_decision2: {
        speaker: "祥哥",
        text: "喔？你說下腳料看起來都是薄管……會不會只是工地隨便堆放的廢料，不一定代表實際埋設的給水管也是薄管？",
        next: "final_decision3",
      },
      final_decision3: {
        speaker: "祥哥",
        text: "老實說，這案子拖很久了，大家壓力都很大。現在也只是從下腳料看到的疑慮，不一定代表實際埋設的管線真的不符規格。如果沒有十足把握，要不要就先不要特別提出來？別把事情弄得太複雜。",
        choices: [
          {
            label: "我認為還是應該先把這個疑點留下紀錄，依程序進一步確認。",
            next: null,
            effects: (s) => {
      ...
      final_no_evidence: {
        speaker: "祥哥",
        text: "現場看起來都沒什麼問題吧？那就先照後續程序辦理囉，辛苦了。",
        choices: [
          {
            label: "好的，沒問題",
            next: null,
  ...
  supervisor: {
    ...
    nodes: {
      greet: {
        ...
        choices: [
          { label: "圖說上指定的給水管規格，工地現場都有照著用嗎？", next: "material_q" },
          { label: "辛苦了，我先四處看看", next: null },
        ],
      },
      material_q: {
        speaker: "監造技師",
        text: "給水管規格喔……施工資料上是照圖說辦理，不過你如果對現場材料有疑問，我可以再一起確認材料證明跟相關紀錄。",
        choices: [
          {
            label: "好，謝謝",
            next: null,
  ...
  contractor: {
    ...
    nodes: {
      greet: {
        ...
        choices: [
          { label: "契約圖說指定要用的給水管規格，工地現場有照著用嗎？", next: "deny" },
          { label: "先看看再說", next: null },
        ],
      },
      deny: {
        speaker: "承包商",
        text: "水管喔……薄一點的也是同類型管材啦，實際使用差不了多少，施工上比較方便，你不用那麼在意啦～",
        choices: [
          { label: "如果實際使用規格與契約或圖說不同，還是應該依契約及相關程序處理，不能由廠商自行更換吧？", next: "gift_setup" },
        ],
      },
      gift_setup: {
        speaker: "承包商",
  ...
  historian: {
    ...
    nodes: {
      ...
      clue: {
        speaker: "文史工作者",
        text: "我最近常來看這邊施工，發現角落堆的管材下腳料好像都是比較薄的規格，沒什麼看到厚一點的管子。\n這個工程的給水管好像有規格要求吧？你們要不要再確認一下。",
        choices: [
          {
            label: "謝謝你告訴我，我會去查證",
            next: null,
  ...
  material_pile: {
    ...
    nodes: {
      inspect: {
        speaker: "（你蹲下來查看）",
        text: "你蹲下來查看角落堆放的管材下腳料……\n外觀上看起來多為管壁較薄的規格，與契約圖說要求的厚管不太一樣，但光憑下腳料還無法直接判定實際埋設的管線是否真的不符規格。\n你拍下現場照片，準備帶回進一步查證。",
        choices: [
          {
            label: "確認",
            next: null,
...
const ENDINGS = {
  hero: {
    title: "誠信守護者",
    text: "你堅持把這個疑點留下紀錄、正式提出，機關隨即啟動後續查核。經相關業務及工程專業人員進一步確認，現場部分管材規格確實與契約要求不符，機關依規定要求廠商改善。同事們也看見了：發現疑慮就依程序說出來，其實沒有想像中可怕。",
  },
  shaky: {
    title: "勇敢的一步",
    text: "你最終選擇正式提出疑點，這是正確的方向。只是過程中有些環節處理得不夠謹慎，讓自己一度陷入尷尬處境。這次的經驗提醒了你：疑慮出現的當下，除了做對選擇，也要把程序走完整，不要留下模糊地帶。",
  },
  compromise: {
    title: "妥協的代價",
    text: "你選擇當作沒看到、沒有留下任何紀錄。半年後，機關在其他工程案件裡重新清查同一批廠商的施工紀錄，這才發現部分管材規格確實與契約要求不符，而你當時其實已經注意到異常，卻沒有處理。這不只是工程問題，也讓你被捲入行政調查——早知道，當初留個紀錄、開口求助就好了。",
  },
  missed: {
    title: "擦肩而過",
    text: "初驗相關作業照原定程序往下進行，你也沒有多想。後來機關在其他查核中重新檢視這項工程，才發現部分給水管規格與契約要求不符。你這才想起當時角落那批下腳料……如果當初多問一句，或許能更早發現問題。",
  },
};
